...
pub const PackageCache = struct {
    ...
    pub fn isPackageCached(self: *PackageCache, package_name: []const u8, version: ?[]const u8) bool {
        ...
        // Create cache key
        const key_buf = std.fmt.allocPrintZ(self.allocator, "{s}-{s}", .{ package_name, version.? }) catch return false;
    ...
    pub fn downloadPackage(self: *PackageCache, package_spec: []const u8, version_arg: ?[]const u8) !void {
        ...
        const pypi_url = try std.fmt.allocPrint(self.allocator, "https://pypi.org/pypi/{s}/json", .{base_name});
        ...
        if (response.status != .ok) {
            try output.printError("Package '{s}' not found on PyPI (status: {d})", .{ package_name, @intFromEnum(response.status) });
        ...
        const cache_key = try std.fmt.allocPrint(self.allocator, "{s}-{s}", .{ base_name, target_version });
        defer self.allocator.free(cache_key);

        if (self.packages.contains(cache_key)) {
            try output.print("Package {s} version {s} already cached", .{ package_name, target_version });
            return;
        }
        ...
        const version_releases = releases.object.get(target_version) orelse {
            try output.printError("Version {s} not found for package {s}", .{ target_version, package_name });
        ...
        if (release_files.items.len == 0) {
            try output.printError("No files found for package {s} version {s}", .{ package_name, target_version });
        ...
        try output.print("Looking for wheels compatible with: {s} / {s}", .{ platform_info.python_tag, platform_info.platform_tag });
        ...
        if (selected_file == null) {
            try output.printError("No suitable distribution found for {s} version {s}", .{ package_name, target_version });
        ...
        // Download the file
        const cache_path = try std.fs.path.join(self.allocator, &[_][]const u8{ self.files_dir, filename.string });
        ...
        try output.print("Successfully cached {s} version {s}", .{ base_name, target_version });
    }
